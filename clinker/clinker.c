...
/* Merge key is LOADNAME, not SEGNAME.  Per GS/OS Reference Appendix F
 ...
 * segment in the load file".  We also keep the existing output
 * segment's KIND — "if segment KINDs are specified in the source file
 * and the KINDs of the object segments placed in a given load segment
 * are not all the same, the segment KIND of the first object segment
 * determines the segment KIND of the entire load segment". */

...
* Ensure ~User_ID is valid before any _NewHandle call.
* GNO exec passes the userID in the A register, but the linker startup
* clobbers A before we get here.  If ~User_ID is still 0, allocate one.
* This must run before the cLine==0 early-return path.
         lda   >~User_ID
         bne   uidok
         pha                                space for result (Word)
         pea   $1000                        application-type userID
         ldx   #$2003                       GetNewID (MiscTool $03, fn $20)
         jsl   $E10000
         pla                                pop result = new userID
         sta   >~User_ID
uidok    anop
         ...
         stz   targv+2
         stz   targv+4                    argv[1] = NULL (terminator)
         stz   targv+6
         brl   rtl                        exit
...
lb6      long  M
         ...
         _NewHandle
         bcc   lb7
         lda   #-1          ; out of memory -- exit silently (no SysLib ~PUTS)
         jml   ~Quit
...
targv    ds    8                        argv[0] + argv[1] null terminator
